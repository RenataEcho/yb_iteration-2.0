...
节假日实物礼。C 端个人中心入口，仅列表。后台：礼品列表 → 点发放名单人数进明细。C 端色板与活动中心一致（浅底 + 橙色）。无卡片动效、无荣誉通知。C 端仅节日列表，已去掉收益历程模板。

| 表面 | 行为 |
|------|------|
| 节日列表 | 封面节日图 + 统一礼物角标、标题、描述、标签。收益礼：进度（当前累计 / 达标，不用「门槛」），详情展示收益统计日期；指定用户礼：定制礼品卡片（文案「特别为你」，不告知圈选规则），无进度条。状态恭喜获得 / 遗憾错失。详情灰区只展示物流 |
| 详情弹窗 | 点卡片底部弹出；弹窗海报、介绍文案、物流单号、缺地址 CTA、进度 |
| 缺地址 | 占名额、标缺地址、不寄；详情点补全地址 → 弹窗「规则：跳转至现在的收获地址页面」 |
| 站内信 | 入选 + 发货各一条，点击都进礼品中心，文案可配 |
| 发放人员 | 新增/编辑单选：指定用户 或 收益达标。指定用户 → C 端定制礼品；收益达标填达标额 + 统计区间。一人一礼一次 |
| 礼物标签 | 后台可配多个，C 端卡片展示全部标签 |
| 礼品图片 | 展示图、详情图、礼品弹窗海报，附件上传（非 URL） |
| 状态 | 仅状态=显示时 C 端才展示；隐藏则前端不展示，礼品仍不下架 |
| 发放名单 | 列表与表单显示命中人数，点击进明细 |
| 礼品列表 | 增列状态（显示/隐藏）、创建人、创建时间 |
| 明细列 | 用户ID、入选方式、收获地址、物流平台、物流单号、当前状态、站内信（未发 / 入选站内信已发 / 发货站内信已发）、更新时间 |
| 明细操作 | 发送入选/发货站内信（仅未发用户，已发跳过）；导入物流单号；补礼品名单 |
| 名单统计 | 顶部三格：入选站内信未发 / 发货站内信未发 / 缺地址 |
| 名单操作 | 筛选项右侧，跟在「查询」后：发入选/发货信、导单号、补礼品 |
| 业务流程 | 圈人、地址、导单号、站内信合在一张判断图 |

礼品不下架。手改物流与满 7 天自动签收并存。
