...
| Surface | 路径 / 入口 | 模板 | 用途 |
|---------|-------------|------|------|
| Sprint 概览 | Sidebar → 迭代 | — | 当前 Sprint FR 清单；已上线归归档，已暂停归废纸篓 |
...
canonical 规范（要点 + 硬限制 + 可点示例）：

→ [`demo/iteration/component-spec-demo.html#sec-flow`](../../../../demo/iteration/component-spec-demo.html#sec-flow)

- **分层渲染**：SVG 连线层（`flow-edges`）置于节点层（`flow-nodes`）下方，避免连线横穿线框。
- **正交路由**：分叉与汇聚使用水平/垂直折线，走节点间隙专用通道，端点必须落在节点边。
- **闭环**：驳回 / 失败 / 重提接到起点顶边；回环禁止贴画布边缘停住。
- **间距**：相邻节点纵向 ≥24px；决策菱形与上下节点 ≥30px。
- **文本**：线框内超 12 字用 `tspan` 换行；副文案降字号、降对比度。
- **默认一张图**：主路径 + 分支 + 回环能闭环，则只保留一个业务流程；拆第二张是例外，不要预置流程 2。
- **Tab 切换**：仅多张时用横向 Tab，单面板仅展示一张，底部共用图例；只有一张图时不要空挂「流程1」。
- **限制**：手写绝对坐标、无自动避让、不做响应式重排、禁用 Mermaid；禁止因节点多就先拆 Tab。
